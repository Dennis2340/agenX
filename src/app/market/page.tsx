  ...
  return (
    <div className="min-h-screen bg-[#f2f7ff] px-6 py-8">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-6 md:hidden h-scroll">
          <div className="h-scroll-inner">
            {items.map((t: any) => (
              <div key={t.id} className="h-card rounded-lg bg-white p-4 shadow">
                <h2 className="text-lg font-semibold text-[#0f3d7a] truncate">{t.title || t.type}</h2>
                <p className="mt-1 text-sm text-[#1e3d63] line-clamp-3">{t.description || 'No description provided.'}</p>
            ...
            {items.length === 0 && (
              <div className="h-card rounded-lg bg-white p-6 text-[#1e3d63]">No tasks yet.</div>
            )}
          </div>
        </div>
